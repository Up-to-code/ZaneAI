...
import { convexZaneAiProRepository } from ".";
...
it("returns a thread shell when the thread exists but messages are not persisted yet", async () => {
  ...
  await expect(convexZaneAiProRepository.getThread("token-1", "thread-123")).resolves.toEqual({
    id: "thread-123",
    title: "Fresh draft",
    messages: [],
  });
...
it("returns null when the thread id is not accessible or does not exist", async () => {
  ...
  await expect(convexZaneAiProRepository.getThread("token-1", "missing-thread")).resolves.toBeNull();
});
