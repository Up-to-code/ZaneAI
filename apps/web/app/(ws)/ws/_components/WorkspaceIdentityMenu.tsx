    ...
    >
      <Link
        ...
        title={`${dictionary.settings.organizationSettingsTitle} · ${organization.name}`}
          className={cn(
            "flex min-w-0 items-center gap-2 rounded-2xl border border-transparent bg-transparent px-2.5 py-2 text-[var(--workspace-bubble-other-foreground)] transition hover:border-[color:var(--workspace-border)] hover:bg-[var(--workspace-elevated)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:color-mix(in_srgb,var(--workspace-highlight)_28%,transparent)]",
            isRtl ? "flex-row-reverse" : "flex-row",
          )}
      >
      ...
      <DropdownMenu>
        <DropdownMenuTrigger
          render={(
            <button
              ...
              className={cn(
                "inline-flex h-11 shrink-0 items-center gap-1.5 rounded-2xl border border-transparent bg-transparent px-2 text-[var(--workspace-bubble-other-foreground)] transition hover:border-[color:var(--workspace-border)] hover:bg-[var(--workspace-elevated)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:color-mix(in_srgb,var(--workspace-highlight)_28%,transparent)]",
                isRtl ? "flex-row-reverse" : "flex-row",
        ...
        <DropdownMenuContent
          align="end"
          sideOffset={8}
          className="w-[min(19rem,calc(100vw-1rem))] min-w-0 rounded-[22px] border border-[color:var(--workspace-border)] bg-[var(--workspace-panel)] p-1 shadow-[0_22px_64px_rgba(10,20,40,0.22)] ring-0"
        >
          <div className="px-1 pb-1">
            <WorkspaceOrganizationSwitcher
              organizationName={organization.name}
              memberLabel={user.email ?? dictionary.settings.member}
            />
          </div>
          {actions.map((action) => (
            <DropdownMenuItem
