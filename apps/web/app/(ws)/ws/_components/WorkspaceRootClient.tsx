...
function WorkspaceLoadingState({ title, description }: { title: string; description: string }) {
  return (
    <div className="flex min-h-[100dvh] flex-col items-center justify-center bg-[var(--zane-ai-background)] p-6 dark:bg-black">
      <div className="relative flex flex-col items-center gap-12">
        ...
        <div className="flex flex-col items-center gap-6 text-center">
          <div className="text-2xl font-black uppercase tracking-[0.5em] text-[var(--zane-ai-deep)] dark:text-white lg:text-4xl">
             {title === "Opening Workspace" ? "Initializing" : title}
          </div>
          ...
          <p className="mt-2 max-w-xs text-[10px] font-black uppercase leading-relaxed tracking-[0.2em] text-[var(--zane-ai-text-muted)] opacity-50">
            {description}
          </p>
        </div>
      </div>
...
function WorkspaceOnboarding({
  ...
  const AUTH_PRIMARY_BUTTON_CLASS_NAME =
    "inline-flex w-full items-center justify-center gap-2 rounded-full bg-[var(--zane-ai-deep)] px-5 py-4 mt-6 text-[13px] tracking-[0.2em] font-black text-white transition-all hover:scale-[1.02] hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:scale-100 dark:bg-white dark:text-black uppercase";

  return (
    <div className="mx-auto flex min-h-[100dvh] w-full items-center justify-center bg-[var(--zane-ai-background)] px-6 py-10 font-sans selection:bg-[var(--zane-ai-deep)]/20 dark:bg-black dark:selection:bg-white/20 sm:px-12">
      <div className="grid w-full max-w-5xl gap-12 lg:grid-cols-2 lg:gap-24">
        ...
        <section className="flex flex-col justify-center text-center lg:text-left" dir="ltr">
          ...
          <h2 className="text-3xl font-black uppercase tracking-widest text-[var(--zane-ai-deep)] dark:text-white sm:text-4xl">
            Initialize
            <br />
            Context
          </h2>
          <p className="mt-6 max-w-md text-sm font-light leading-relaxed tracking-widest text-[var(--zane-ai-text-muted)] dark:text-white/50">
          ...
          <div className="mx-auto w-full max-w-md lg:mx-0">
            ...
            {pendingInvites.length > 0 ? (
              <div className="mb-12 space-y-6">
                <div>
                  <div className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--zane-ai-text-muted)] dark:text-white/40">Pending Invites</div>
                </div>
                {pendingInvites.map((invite) => (
                  <div key={invite.id} className="relative overflow-hidden border-b border-[var(--zane-ai-line)] pb-6 transition-colors dark:border-white/10">
                    <div className="text-xl font-black text-[var(--zane-ai-deep)] dark:text-white">{invite.organizationName}</div>
                    <div className="mt-2 text-[10px] font-black uppercase tracking-widest text-[var(--zane-ai-text-muted)] dark:text-white/40">
                      Dispatched by {invite.inviterName} • <span className="text-[var(--zane-ai-deep)] dark:text-white/80">{invite.role}</span>
                    </div>
                    <button
                      type="button"
            ...
            >
              <div>
                <div className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--zane-ai-text-muted)] dark:text-white/40">Owner Genesis</div>
                <h2 className="mt-2 text-2xl font-black uppercase tracking-widest text-[var(--zane-ai-deep)] dark:text-white">Create Org</h2>
              </div>
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-4">
                  <span className="block pb-2 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--zane-ai-text-muted)] dark:text-white/40">Entity Type</span>
                    ...
                    <button
                      ...
                      className={`relative overflow-hidden border-b-2 px-2 py-4 text-left text-[11px] font-black uppercase tracking-[0.2em] transition-all ${
                        type === "red"
                          ...
                          : "border-transparent text-[var(--zane-ai-text-muted)]/50 hover:text-[var(--zane-ai-text-muted)] dark:border-transparent dark:text-white/40 dark:hover:text-white"
                      }`}
                    >
                      Developer
                    </button>
