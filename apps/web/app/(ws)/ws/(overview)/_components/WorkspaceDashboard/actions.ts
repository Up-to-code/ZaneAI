...
} from "@/server/domains/workspace/zaneAiPro/service";
import {
  sendZaneAiProMessageInputSchema,
  transcribeVoiceFromStorageInputSchema,
  type ZaneAiProThread,
  type SendZaneAiProMessageInput,
 ...
 * HOW:   Validates with the shared schema then delegates to the ZaneAiPro domain service.
 */
export async function sendAssistantMessage(
  input: SendZaneAiProMessageInput,
): Promise<WorkspaceActionResult<ZaneAiProThread>> {
  try {
    const parsed = sendZaneAiProMessageInputSchema.safeParse(input);
    if (!parsed.success) {
    ...
    const data = await sendZaneAiProMessage(parsed.data);
    return { ok: true, data };
  } catch (error) {
 ...
 * HOW:   Delegates to the ZaneAiPro voice domain service and normalizes errors for client handling.
 */
export async function getVoiceUploadUrl(): Promise<WorkspaceActionResult<{ uploadUrl: string }>> {
  try {
    const uploadUrl = await getZaneAiProVoiceUploadUrl();
    return {
      ok: true,
      data: { uploadUrl },
    };
...
export async function transcribeVoiceFromStorage(
  ...
  try {
    ...
    const data = await transcribeZaneAiProVoiceFromStorage(parsed.data);
    return { ok: true, data };
  } catch (error) {
    return toActionError(error);
  }
...
export async function finalizeAssistantUploads(
  ...
  try {
    ...
    const data = await finalizeZaneAiProUploadedFiles(parsed.data);
