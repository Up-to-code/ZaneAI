...
export type AssistantStreamEvent =
  ...
  | { event: "done"; data: { thread: ZaneAiProThread } }
...
export function parseSseChunk(rawChunk: string): AssistantStreamEvent | null {
  ...
  try {
    ...
    if (event === "done") return { event, data: data as { thread: ZaneAiProThread } };
...
export function getAssistantMotionState(
  isStreaming: boolean,
  streamStage: ZaneAiProStreamStageEvent | null,
): AIMotionState {
