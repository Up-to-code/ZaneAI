...
import type { ZaneAiProInputMode, ZaneAiProThread } from "@/server/contracts/zaneAiPro";
...
function createOptimisticThread(args: {
  previousThread: ZaneAiProThread | null;
  ...
  const optimisticThread: ZaneAiProThread = args.previousThread ?? { id: "", title: null, messages: [] };
...
async function requestAssistantStream(args: {
  ...
  const response = await fetch("/api/workspace/zane-ai-pro?stream=1", {
    method: "POST",
