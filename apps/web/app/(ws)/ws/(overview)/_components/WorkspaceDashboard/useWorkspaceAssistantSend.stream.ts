...
import type { ZaneAiProStreamStageEvent, ZaneAiProThread } from "@/server/contracts/zaneAiPro";
import {
  parseSseChunk,
...
export type StreamSetters = {
  setThread: React.Dispatch<React.SetStateAction<ZaneAiProThread | null>>;
  setSelectedThreadId: React.Dispatch<React.SetStateAction<string | null>>;
  setStreamStage: React.Dispatch<React.SetStateAction<ZaneAiProStreamStageEvent | null>>;
  setStageHistory: React.Dispatch<React.SetStateAction<ZaneAiProStreamStageEvent[]>>;
...
export type StreamRunState = {
  ...
  optimisticThread: ZaneAiProThread;
...
function setAssistantMessageState(args: {
  ...
  optimisticThread: ZaneAiProThread;
  assistantMessageId: string;
  apply: (message: ZaneAiProThread["messages"][number]) => ZaneAiProThread["messages"][number];
}) {
...
function updateStageMeta(
  stage: ZaneAiProStreamStageEvent,
  setters: StreamSetters,
) {
 ...
 */
export function buildStreamState(assistantMessageId: string, optimisticThread: ZaneAiProThread): StreamRunState {
  return {
    assistantMessageId,
    optimisticThread,
    assistantText: "",
