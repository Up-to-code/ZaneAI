...
const { requireWorkspaceData, getZaneAiProThread, listIncomingOrganizationInvitesForCurrentUser } = vi.hoisted(() => ({
  requireWorkspaceData: vi.fn(),
  getZaneAiProThread: vi.fn(),
...
it("renders blank draft mode for /ws without creating a thread", async () => {
  ...
  expect(getZaneAiProThread).not.toHaveBeenCalled();
...
it("loads an existing thread when threadId is valid", async () => {
  getZaneAiProThread.mockResolvedValue({
    id: "thread-123",
    title: "Saved",
    messages: [],
  });
  ...
  expect(getZaneAiProThread).toHaveBeenCalledWith("thread-123");
...
it("renders a recoverable blank state when threadId is invalid", async () => {
  getZaneAiProThread.mockResolvedValue(null);
