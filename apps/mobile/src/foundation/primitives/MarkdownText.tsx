 ...
 */
export function MarkdownText({ text, style, tone = "secondary" }: MarkdownTextProps) {
  const renderedContent = useMemo(() => {
    ...
    lines.forEach((line, lineIdx) => {
      ...
      const renderedLine = parseInlineMarkdown(line, lineIdx);
      ...
      if (isListItem) {
        elements.push(
          <View key={lineIdx} style={[styles.listItem, isAr && { flexDirection: "row-reverse" }]}>
            <Text tone={tone} selectable={true} style={[styles.baseText, isAr && { textAlign: "right" }, style]}>
              {renderedLine}
            </Text>
          </View>
        );
      ...
      elements.push(
        <Text
          ...
          tone={tone}
          selectable={true}
          style={[styles.baseText, style, lineIdx < lines.length - 1 && styles.paragraph, isAr && { textAlign: "right" }]}
        >
          {renderedLine}
        </Text>
      );
...
const styles = StyleSheet.create({
  ...
  hashtag: {
    color: "#6366f1",
    fontFamily: "Manrope_700Bold",
    // No explicit font size, will inherit from baseText or parent style
  },
});
