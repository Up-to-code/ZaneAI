...
const projectInputValidator = {
  ...
  amenitiesText: v.optional(v.string()),
  hasParking: v.optional(v.boolean()),
  parkingSpaces: v.optional(v.string()),
  coverImageKey: v.optional(v.union(v.string(), v.null())),
  galleryDisplayMode: v.optional(galleryDisplayModeValidator),
  galleryAspectRatio: v.optional(galleryAspectRatioValidator),
  privatePermitSummary: v.optional(v.string()),
  privatePermitFiles: v.optional(v.array(uploadedFileReference)),
  rooms: v.optional(v.string()),
  baths: v.optional(v.string()),
  area: v.optional(v.string()),
  status: v.optional(v.string()),
  expectedUnits: v.optional(v.string()),
  clientVisibility: v.optional(clientVisibilityValidator),
  images: v.optional(v.array(uploadedFileReference)),
  ...
  // Egyptian Market Project Fields
  startingPrice: v.optional(v.string()),
  installmentYears: v.optional(v.string()),
...
export const createWorkspaceProperty = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const projectId = await ctx.db.insert("projects", {
      ...
      shortDescription: (args.shortDescription ?? args.description).trim(),
      priceLabel: toPriceLabel(args.price ?? args.startingPrice ?? "0"),
      startingPrice: args.startingPrice,
      installmentYears: args.installmentYears ? Number(args.installmentYears) || undefined : undefined,
      ...
      currency: args.currency as "EGP" | "USD" | undefined,
      maintenanceFees: args.maintenanceFees,
...
export const updateWorkspaceProperty = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(args.propertyId, {
      ...
      shortDescription: (args.data.shortDescription ?? args.data.description).trim(),
      priceLabel: toPriceLabel(args.data.price ?? args.data.startingPrice ?? "0"),
      expectedUnits: args.data.expectedUnits ? Number(args.data.expectedUnits) || undefined : undefined,
      startingPrice: args.data.startingPrice,
      installmentYears: args.data.installmentYears ? Number(args.data.installmentYears) || undefined : undefined,
      ...
      updatedAt: now,
      compoundName: args.data.compoundName,
      unitCode: args.data.unitCode,
      direction: args.data.direction,
      currency: args.data.currency as "EGP" | "USD" | undefined,
      maintenanceFees: args.data.maintenanceFees,
      monthlyInstallment: args.data.monthlyInstallment,
